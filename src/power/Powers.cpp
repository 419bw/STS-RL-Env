...
void VulnerablePower::onApply(GameEngine& engine) {
    std::weak_ptr<AbstractPower> weakSelf = shared_from_this();

    engine.eventBus.subscribe(EventType::ON_ROUND_END,
        [weakSelf](GameEngine& eng, void*) -> bool {
            ...
            if (self->getAmount() == 0) {
                eng.actionManager.addAction(std::make_unique<RemoveSpecificPowerAction>(self->getOwner(), self));
            } else {
                eng.actionManager.addAction(std::make_unique<ReducePowerAction>(self->getOwner(), self, 1));
            }
            return true;
        });
}
...
// - 回合开始时层数 -1
// ==========================================
void PoisonPower::onApply(GameEngine& engine) {
    std::weak_ptr<AbstractPower> weakSelf = shared_from_this();

    engine.eventBus.subscribe(EventType::ON_TURN_START,
        [weakSelf](GameEngine& eng, void* context) -> bool {
            auto self = weakSelf.lock();
            if (!self) {
                return false;
            }
            ...
            if (current_turn_char == self->getOwner().get()) {
                if (self->getAmount() == 0) {
                    ...
                    eng.actionManager.addAction(std::make_unique<LoseHpAction>(self->getOwner(), self->getAmount()));
                    eng.actionManager.addAction(std::make_unique<ReducePowerAction>(self->getOwner(), self, 1));
                }
            }
            return true;
