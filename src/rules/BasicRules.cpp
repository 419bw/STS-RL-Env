...
void BasicRules::registerRules(GameEngine& engine) {
    ...
    engine.eventBus.subscribe(EventType::PHASE_BATTLE_START,
        [&engine](GameEngine& eng, void*) -> bool {
            //重置怪物意图
            engine.actionManager.addAction(std::make_unique<ResetAllBrainsAction>());
            STS_LOG(*engine.combatState, "    [BasicRules] 战斗开始 -> 洗牌 + 刷新怪物意图\n");
            engine.actionManager.addAction(std::make_unique<ShuffleDiscardIntoDrawAction>());
            //刷新怪物意图
            engine.actionManager.addAction(std::make_unique<RollAllMonsterIntentsAction>());
            return true;
        });
    ...
    engine.eventBus.subscribe(EventType::PHASE_PLAYER_TURN_START,
        [&engine](GameEngine& eng, void*) -> bool {
            STS_LOG(*engine.combatState, "    [BasicRules] 玩家回合开始 -> 抽 5 张牌\n");
    ...
    engine.eventBus.subscribe(EventType::PHASE_PLAYER_TURN_END,
        [&engine](GameEngine& eng, void*) -> bool {
            STS_LOG(*engine.combatState, "    [BasicRules] 玩家回合结束 -> 弃置所有手牌\n");
    ...
    engine.eventBus.subscribe(EventType::PHASE_MONSTER_TURN_END,
        [&engine](GameEngine& eng, void*) -> bool {
            STS_LOG(*engine.combatState, "    [BasicRules] 怪物回合结束 -> 刷新怪物意图\n");
            engine.actionManager.addAction(std::make_unique<RollAllMonsterIntentsAction>());
            return true;
        });
    ...
    engine.eventBus.subscribe(EventType::PHASE_MONSTER_TURN,
        [&engine](GameEngine& eng, void*) -> bool {
            for (auto& monster : engine.combatState->monsters) {
                if (!monster->isDead()) {
                    STS_LOG(*engine.combatState, "    [BasicRules] " << monster->name << " 执行回合\n");
                    engine.actionManager.addAction(std::make_unique<MonsterTakeTurnAction>(monster));
                }
            }
            return true;
        });
