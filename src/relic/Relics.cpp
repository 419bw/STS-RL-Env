...
void CustomVajraRelic::onEquip(GameEngine& engine, Character* target) {
    ...
    engine.eventBus.subscribe(EventType::ON_CARD_PLAYED,
        [weakSelf](GameEngine& eng, void* context) -> bool {
            ...
            if (playedCard->type == CardType::ATTACK) {
                STS_LOG(*eng.combatState, "[" << self->name << "] 触发！这是一张攻击牌，产生额外效果。\n");
            }
            return true;
        });
}
...
void ChemicalXRelic::onEquip(GameEngine& engine, Character* target) {
    ...
    engine.eventBus.subscribe(EventType::ON_CARD_PLAYING,
        [weakSelf](GameEngine& eng, void* context) -> bool {
            auto self = weakSelf.lock();
            if (!self) {
                return false;
            ...
            if (playingCard->cost == -1) {
                ...
                STS_LOG(*eng.combatState, "[" << self->name << "] 触发！为 X 费牌注入额外能量。当前实际释放能量: "
