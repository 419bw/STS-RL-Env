...
| 签名 | 说明 |
|------|------|
| `static std::unique_ptr<LambdaAction> make(std::weak_ptr<Character> source, std::function<void(GameState&, Character*)> closure)` | 工厂函数，创建带 source 生命周期监听的 LambdaAction |

**闭包类型**：`std::function<void(GameState&, Character* source)>`
